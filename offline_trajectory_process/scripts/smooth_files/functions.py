...
def print2D(raw_coords,smooth_coords,p=[],case=0,outliers=[],filtered=[]):
	...
	# ax.set_aspect('equal')
	ax[0].grid()
	ax[0].scatter(np.array(raw_coords[0]),np.array(raw_coords[1]),alpha=0.3,label="Initial Points")
	ax[0].plot(np.array(smooth_coords[0]),np.array(smooth_coords[1]),alpha=0.3,label="Initial Points")
	ax[0].set_xlabel('x(m)')
	ax[0].set_ylabel('y(m)')

	ax[1].grid()	
	ax[1].scatter(np.array(raw_coords[1]),np.array(raw_coords[2]),alpha=0.3,label="Initial Points")
	ax[1].plot(np.array(smooth_coords[1]),np.array(smooth_coords[2]),alpha=0.3,label="Initial Points")
	ax[1].set_xlabel('y(m)')
	ax[1].set_ylabel('z(m)')
	

	ax[2].grid()
	ax[2].scatter(np.array(raw_coords[0]),np.array(raw_coords[2]),alpha=0.3,label="Initial Points")
	ax[2].plot(np.array(smooth_coords[0]),np.array(smooth_coords[2]),alpha=0.3,label="Initial Points")
	ax[2].set_xlabel('x(m)')
	ax[2].set_ylabel('z(m)')
